...
def login_required(f):
    ...
    async def decorated_function(*args, **kwargs):
        ...
        if not magic_bits:
            return jsonify({"authenticated": False, "error": "The Dragon roared too Loudly for your Magic."}), 401

        # 2. Database Verification
        is_valid, error = UserDriver._validate_token(user_id, magic_bits)
        if (not is_valid) or error:
            return jsonify({"authenticated": False, "error": f"Invalid session token. Hint: {error}"}), 401
